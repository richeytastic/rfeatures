#include "AAMReader.h"
using RFeatures::AAMInfoReader;
using RFeatures::AAMReader;
    ...
    : _cps(aamInfo)
{
    ...
    _offset = lasReader.getOffset();
    _scale = lasReader.getScale();

    // Find the indices into the capture points array that's relevant for this LAS file
    const double minTime = lasReader.getMinTime();
    const double maxTime = lasReader.getMaxTime();
    const double minX = lasReader.getMinX();
    const double minY = lasReader.getMinY();
    const double minZ = lasReader.getMinZ();
    const double maxX = lasReader.getMaxX();
    const double maxY = lasReader.getMaxY();
    const double maxZ = lasReader.getMaxZ();
    /*
...
Panorama::Ptr AAMReader::createPanorama( int idx) const
{
    Panorama::Ptr pano;
    ...
    pano = Panorama::Ptr( new Panorama( pos, upv, cp.yaw, cp.pitch, cp.roll));
    assert( pcs.size() == 4);
